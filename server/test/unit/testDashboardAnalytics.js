...
const cleanupTestData = async () => {
    ...
    if (testUserId) {
        ...
        console.log(
            `[CLEANUP] 4. Deleted ${userDelete.deletedCount} test User document(s).`
        );
    }
...
const runDashboardAnalyticsTest = async () => {
    ...
    try {
        const testUser = await new User({
            email: `analytics_test_${timestamp}@revenuerescue.ai`,
        ...
        for (let i = 0; i < retryOutcomes.length; i++) {
            ...
            const ev = new Event({
                ...
                paymentAmount: 1000,
                status:
                    retryOutcomes[i] === "RECOVERED"
                        ? "RECOVERED"
                        : "RESOLVED_UNRECOVERED",
        ...
        // B) SEND_PAYMENT_LINK + INSUFFICIENT_FUNDS:
        // 1 RECOVERED, 1 FAILED -> Probability = 1/2
        const linkOutcomes = [
            "RECOVERED",
            "FAILED"
        ];
        ...
        console.log(
            "  - RETRY_NOW + NETWORK_ERROR: 2 RECOVERED, 1 FAILED (Prob = 2/3)"
        );
        console.log(
            "  - SEND_PAYMENT_LINK + INSUFFICIENT_FUNDS: 1 RECOVERED, 1 FAILED (Prob = 0.5)"
        );

        // STEP 2: Create currently active failed events

        // 1) Eligible Failed Event 1:
        // paymentAmount = 9000, NETWORK_ERROR, RETRY_NOW -> EV = 6000
        ...
        // 2) Eligible Failed Event 2:
        // paymentAmount = 4000, INSUFFICIENT_FUNDS,
        // SEND_PAYMENT_LINK -> EV = 2000
        ...
        console.log(
            "[SETUP] Created 5 active failed events (2 eligible: ₹9,000 and ₹4,000; 3 excluded)."
        );

        // STEP 3: Execute scoped dashboard metrics
        console.log(
            "\n--- EXECUTING getDashboardMetrics(testUserId, trackedEventObjectIds) ---"
        );

        const metrics = await getDashboardMetrics(
            testUserId,
            trackedEventObjectIds
        );

        console.log(
            "Scoped Dashboard Metrics Output:",
        ...
        console.log(
            `1. expectedRecoveryValue is present: ${metrics.expectedRecoveryValue !== undefined}`
        );

        if (metrics.expectedRecoveryValue === undefined) {
            throw new Error(
                "expectedRecoveryValue is undefined in getDashboardMetrics response"
            );
        }

        console.log(
            `2. expectedRecoveryValue === ${expectedEV}: ${Math.abs(metrics.expectedRecoveryValue - expectedEV) < 1e-9}`
        );

        if (
            Math.abs(metrics.expectedRecoveryValue - expectedEV) >=
            1e-9
        ) {
            throw new Error(
                `Expected EV ${expectedEV}, got ${metrics.expectedRecoveryValue}`
            );
        }

        console.log(
            `3. failedPayments count is tracked: ${metrics.failedPayments} (Expected: 5)`
        );

        if (metrics.failedPayments !== 5) {
            throw new Error(
                `Expected 5 failed payments in fixture, got ${metrics.failedPayments}`
            );
        }

        console.log(
            `4. recoveredPayments count is tracked: ${metrics.recoveredPayments} (Expected: 3)`
        );

        if (metrics.recoveredPayments !== 3) {
            throw new Error(
                `Expected 3 recovered payments in fixture, got ${metrics.recoveredPayments}`
            );
        }

        const expectedRate = 3 / (3 + 2);

        console.log(
            `5. recoveryRate is tracked: ${metrics.recoveryRate} (Expected: ${expectedRate})`
        );

        if (
            Math.abs(metrics.recoveryRate - expectedRate) >=
            1e-9
        ) {
            throw new Error(
                `Expected recoveryRate ${expectedRate}, got ${metrics.recoveryRate}`
            );
        }
...
runDashboardAnalyticsTest().catch((err) => {
    console.error(
        "Dashboard Analytics Test failed with error:",
        err
    );
